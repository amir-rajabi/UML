...
revert.addEventListener('change', function(){
    ...
    if (revert.checked) {
        if (!dsa_revert_flag){
            start_beta_train.setAttribute('data-bs-toggle', 'modal');
            start_beta_train.setAttribute('data-bs-target', '#revertModal');
        }
        else {
            start_beta_train.removeAttribute('data-bs-toggle');
            start_beta_train.removeAttribute('data-bs-target');
        }
    } else {
        start_beta_train.removeAttribute('data-bs-toggle');
        start_beta_train.removeAttribute('data-bs-target');
...
revert_confirmed.addEventListener('click', function(){
    ...
    start_beta_train.style.display = 'none';
    stop_beta_train.style.display = 'block';
    var xhr = new XMLHttpRequest();
    xhr.open('POST', '/start_beta', true);
    xhr.setRequestHeader('Content-Type', 'application/json');
    xhr.send(true);
    startProgress();
    block_while_running();
...
start_beta_train.addEventListener('click', function(){
...
stop_beta_train.addEventListener('click', function(){
    start_beta_train.style.display = 'block';
    stop_beta_train.style.display = 'none';
    block_button(start_beta_train, true);
    var xhr = new XMLHttpRequest();
    xhr.open('POST', '/stop_beta', true);
    xhr.setRequestHeader('Content-Type', 'application/json');
    xhr.send(1);
    stopProgress();
});
...
socket.on('training_finished', function(data){
    ...
    block_button(start_beta_train, false);
    start_beta_train.style.display = 'block';
    stop_beta_train.style.display = 'none';
